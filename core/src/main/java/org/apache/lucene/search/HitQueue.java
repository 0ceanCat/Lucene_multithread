...
public final class HitQueue extends PriorityQueue<ScoreDoc> {
    ...
    public List<ScoreDoc> getDocs() {
        Iterator<ScoreDoc> iterator = iterator();
        List<ScoreDoc> l = new ArrayList<>();
        while (iterator.hasNext()) {
            ScoreDoc next = iterator.next();
            if (next.doc != Integer.MAX_VALUE) l.add(next);
        }
        return l;
    }

    public List<ScoreDoc> getDocs2() {
        Iterator<ScoreDoc> iterator = iterator();
        List<ScoreDoc> l = new ArrayList<>();
        while (iterator.hasNext()) {
            ScoreDoc next = iterator.next();
